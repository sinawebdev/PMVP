...
Phase 1 MVP for Chrisnat Limited Ghana. This is a Flask web app that bridges Excel payroll files into a simple review, approval, accounts, and export workflow. It uses SQLite locally by default and Render PostgreSQL when `DATABASE_URL` is configured.
...
- `AUTO_INIT_DB`: leave as `true` for MVP auto table creation/seed, or set `false` and run `flask --app run:app init-db` manually.

The app normalizes old-style `postgres://` URLs to `postgresql://` for SQLAlchemy. `db.create_all()` and additive schema checks are used for MVP-safe table setup; they do not wipe production data. For a later production phase, replace this with Flask-Migrate migrations.
